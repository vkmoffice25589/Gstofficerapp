...
function renderDownloadsPage() {
  ...
  AppState.bankAtts.forEach(function (b) {
    rows.push({ date: b.createdAt, type: 'Bank Attachment — Letter to Bank', icon: 'fa-building-columns', color: 'gold', gstin: b.gstin, name: b.legalName, amt: b.totalAmt, action: function () { return buildBankLetterDocx(b, getSettings()).then(function (blob) { downloadBlob(blob, b.ref.replace(/\//g, '_') + '_Letter.docx'); }); }, actionPdf: function () { generateBankLetterPDF(b, getSettings()); } });
    rows.push({ date: b.createdAt, type: 'Bank Attachment — Form DRC-13', icon: 'fa-building-columns', color: 'gold', gstin: b.gstin, name: b.legalName, amt: b.totalAmt, action: function () { return buildBankDrc13Docx(b, getSettings()).then(function (blob) { downloadBlob(blob, b.ref.replace(/\//g, '_') + '_DRC13.docx'); }); }, actionPdf: function () { generateBankDrc13PDF(b, getSettings()); } });
